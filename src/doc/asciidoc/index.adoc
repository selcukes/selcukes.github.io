...
Add `selcukes.yaml` file in `src/test/resource` folder and update below options as required
...
Add `selcukes.yaml` file in resource folder and update below options as required
...
NOTE: Selcukes DataBind is a transitive Dependency of `webdriver-binaries`.If you are using `webdriver-binaries`, then ignore adding this dependency explicitly.
...
[source,java]
----
include::../../test/java/io/github/selcukes/tests/databind/CreateDataFileTest.java[tags=snippet-in-doc,indent=0]
----
...
NOTE: Selcukes Extent Reports is a transitive Dependency of `selcukes-reports`.If you are using `selcukes-reports`, then ignore adding this dependency explicitly.

WARNING: If you are using `selcukes-testng` then refer <<_selcukes_testng, Selcukes TestNG>> for extent report integration
...
[.thumb]
.Notification
image::teams-notification.png[scaledwidth=100%]
...
https://github.com/selcukes/selcukes-java/tree/master/selcukes-testng[Selcukes TestNG] is used to run cucumber testng tests with easy runtime configurations for different modules.

=== Setup

Selcukes TestNG used as a Java dependency in Maven as follows
...
In the case of a Gradle project, we can declare Selcukes TestNG as follows:

[source,json,subs="attributes+"]
----
dependencies {
    implementation("io.github.selcukes:selcukes-testng:{project-version}")
}
----
...
[source,yaml]
----
include::../../test/resources/selcukes.yaml[lines=11..16]
----
...
----
include::../../test/resources/selcukes.yaml[lines=28..31]
----

NOTE: Here emailReport means extent reports, which will created automatically without any additional configurations.

Create your own Test runner by extending `SelcukesTestNGRunner`

Add By default, all scenarios will execute in sequential.
To enable scenario to run in parallel use `@parallel` annotation to the scenarios as follows
...
Selcukes Reports allows record video of your tests with minimal configurations

[source,yaml]
----
include::../../test/resources/selcukes.yaml[lines=32..36]
----

==== FFMPEG Installation

In order to use ffmpeg type recorder first you need to follow below steps:
...
- Download ffmpeg and extract to folder
+
Ex: C:\ffmpeg
- Add FFMPEG bin to PATH variable.
+
Ex: C:\ffmpeg\bin
- Download `SendSignalCtrlC.exe` from https://www.ffmpeg.org/download.html[here] and put into the ffmpeg bin folder
+
Ex: C:\ffmpeg\bin
...
[.thumb]
.FFMPEG
image::ffmpeg.JPG[scaledwidth=100%]

[[sendCtrlC]]
[.thumb]
.SendCtrlC
image::sendCtrlC.JPG[scaledwidth=100%]
