...
==== Java 11
Test Automation is a development activity, so you will need some familiarity with Java. Selcukes uses Java 11, so make sure you have a http://www.oracle.com/technetwork/java/javase/downloads/index.html[JDK 11 or later] installed.
...
[source,yaml]
----
include::../../main/resources/selcukes.yaml[lines=9..13]
----
...
[source,yaml]
----
include::../../main/resources/selcukes.yaml[lines=14..17]
----
Create Sample Notepad Test as follows
[source,java]
----
...
[source,yaml]
----
include::../../main/resources/selcukes.yaml[lines=18..20]
----
Create Sample Browser Test as follows
[source,java]
----
...
WebDriver Binaries provides a set of _binaries_ for Chrome, Firefox, Edge, Opera, Chromium, and Internet Explorer. The basic use of these binary is the following:
...
WebDriver Binaries executes a _resolution algorithm_ when calling to `setup()` in a given manager. The most relevant parts of this algorithm are the following:
...
=== Data Parser
==== Read Json/Yml files
Create Test data file as `test-users.json` in resource folder
[source,json]
----
{
  "users": [
...
----
Here is an example class to demonstrate to parse  and read `test-users.json`
[source,java]
----
include::../../test/java/io/github/selcukes/tests/databind/DataMapperTest.java[tags=snippet-in-doc,indent=0]
----
==== Update Json/Yml files
Here is another example to update test file in runtime

Consider we have `test_sample.yml` located in resource folder as follows
[source,yml]
----
users:
  user1:
...
----
Here is an example class to demonstrate to update `test_sample.yml`

[source,java]
----
include::../../test/java/io/github/selcukes/tests/databind/DataMapperWriteTest.java[tags=snippet-in-doc,indent=0]
----
...
[source,yaml]
----
include::../../main/resources/selcukes.yaml[lines=26..31]
----
...
[source,yaml]
----
include::../../main/resources/selcukes.yaml[]
----
=== Runtime Properties
...
----
== Community
There are two ways to try to get community support related to Selcukes. First, questions about it can be discussed in https://stackoverflow.com/questions/tagged/selcukes_java[StackOverflow], using the tag _selcukes_java_. In addition, comments, suggestions, and bug-reporting should be made using the https://github.com/selcukes/selcukes-java/issues[GitHub issues]. Finally, if you think Selcukes can be enhanced, consider contributing to the project through a  https://github.com/selcukes/selcukes-java/pulls[pull request].

== Support
https://opencollective.com/selcukes[Selcukes] is part of https://opencollective.com/selcukes[OpenCollective], an online funding platform for open and transparent communities. You can support the project by contributing as a backer (i.e., a personal https://opencollective.com/selcukes/donate[donation] or https://opencollective.com/selcukes/contribute/backer-8132/checkout[recurring contribution]) or as a https://opencollective.com/selcukes/contribute/sponsor-8133/checkout[sponsor] (i.e., a recurring contribution by a company).
...
== About
Selcukes (Copyright © 2019-2022) is an open-source project created and maintained by https://github.com/RameshBabuPrudhvi/[Ramesh Babu Prudhvi] (https://twitter.com/amrameshbabu[@amrameshbabu]), licensed under the terms of https://www.apache.org/licenses/LICENSE-2.0[Apache 2.0 License]. This documentation (also available in link:selcukes.pdf[PDF] and link:selcukes.epub[EPUB]) is released under the terms of https://creativecommons.org/licenses/by-nc-sa/2.0/[CC BY-NC-SA 2.0].
